...
import { priceLockSubject, priceLockHtml } from "./priceLockEmail";
...
// ── Price-lock welcome email (ZIP step lead capture) ────────────
// Fired automatically by /api/price-lock. Reply-To is hello@ so
// questions about the move land in the team inbox.
export async function sendPriceLockEmail({ email, lockedUntilText }) {
  const { data, error } = await resend.emails.send({
    from: FROM_EMAIL,
    to: email,
    reply_to: "hello@roomonyc.com",
    subject: priceLockSubject(),
    html: priceLockHtml({ lockedUntilText }),
  });
  if (error) {
    console.error("Resend price-lock email error:", error);
    throw new Error(error.message || "Failed to send price-lock email");
  }
  return data;
}
