...
// ─── Living / Sofa rental state ─────────────────────────────────
// Shared global state read by the cart (RoomoCart / RoomoCartIcon)
// via window.__rentalState

function setRentalState(patch: any) {
    if (typeof window === "undefined") return
    if (!(window as any).__rentalState) {
        ;(window as any).__rentalState = {
            months: 12,
            hasLamp: true,
            hasPlant: true,
        }
        ;(window as any).__rentalListeners = new Set()
    }
    ;(window as any).__rentalState = {
        ...(window as any).__rentalState,
        ...patch,
    }
    ;(window as any).__rentalListeners.forEach((fn: () => void) => fn())
...
function useRentalState() {
    ...
    useEffect(() => {
        ...
        const listeners = (window as any).__rentalListeners as Set<() => void>
    ...
    return (window as any).__rentalState
}
            ...
            return (
                <div
                    ...
                    style={{
                        ...props.style,
                        position: "absolute",
                        left: `calc(100% - ${props.style?.width ?? 16}px)`,
                    }}
                />
            )
        }
